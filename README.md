...
- **Explore Trusts** — the nine core lifetime-transfer vehicles (SLAT, SLANT,
  ILIT, GRAT, IDGT, Dynasty, CRT, QPRT, CLAT). Lineup grid → full "product page"
  per trust → select up to three and compare side by side.
- **Situational Trusts** — the seven at-death / special-purpose structures
  (Credit Shelter, QTIP, QDOT, SNT, DAPT, BDIT, ING), which use their own
  schema.
...
- **Situation Guide** — 19 client situations. Select up to five and print a
  research brief: trusts to research, why they fit, and the watch-outs.
- **Objectives Guide** — the 20-row decision tree: name the objective, get the
  structure to start with and why.

Plus a **Reference** area (key federal figures, GST & ETIP explainers, state
panel, 40-term glossary) that the rest of the site links into. Jargon gets
...
(`scripts/Trust_Structures_Lookup_v2_3.xlsx`), extracted to JSON in
